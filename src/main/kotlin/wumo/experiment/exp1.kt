...
//            Thread.sleep(1000L)
    ...
    val stepListener: StepListener = step@{ episode, step, s, a ->
      ...
      D2DGameUI.render { gc ->
        ...
        with(s) {
          for (stage in 0 until wumo.problem.FlyPlane.maxStage) {
            gc.fill = javafx.scene.paint.Color.GREEN
            gc.fillOval((wumo.problem.FlyPlane.target.loc.x - wumo.problem.FlyPlane.target.radius).transX(stage),
                        (wumo.problem.FlyPlane.target.loc.y - wumo.problem.FlyPlane.target.radius).transY(stage),
                        2 * wumo.problem.FlyPlane.target.radius.tranUnit(),
                        2 * wumo.problem.FlyPlane.target.radius.tranUnit())
            gc.fill = javafx.scene.paint.Color.GREY
            for (i in 0 until wumo.problem.FlyPlane.numObstaclesPerStage) {
              val obstacle = wumo.problem.FlyPlane.stageObstacles[stage][i]
          ...
          gc.fill = javafx.scene.paint.Color.ORANGE
          val dir = s.vel.copy().norm() * wumo.problem.FlyPlane.plane.radius
          val top = s.loc + dir
          val left = s.loc + (dir / 2.0).rot90L()
          val right = s.loc + (dir / 2.0).rot90R()
          gc.fillPolygon(kotlin.doubleArrayOf(top.x.transX(stage), left.x.transX(stage), right.x.transX(stage)),
                         kotlin.doubleArrayOf(top.y.transY(stage), left.y.transY(stage), right.y.transY(stage)),
                         3)
        }
        for (stage in 0 until FlyPlane.maxStage) {
